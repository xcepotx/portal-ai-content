# yt-automation-onefact-ind/tabs/real_estate_studio.py
...
def _build_outputs_zip(job_dir: Path) -> Path:
    ...
    zip_path = (job_dir / "outputs" / f"real_estate_{job_dir.name}.zip").resolve()
...
def _ensure_defaults():
    ...
    st.session_state.setdefault(f"{TAB_KEY}_styles", ["Minimalist", "Scandinavian"])
    st.session_state.setdefault(f"{TAB_KEY}_room_type", "Living room")
    st.session_state.setdefault(f"{TAB_KEY}_staging_strength", "Medium")
    st.session_state.setdefault(f"{TAB_KEY}_lighting_fix", True)
    st.session_state.setdefault(f"{TAB_KEY}_declutter", True)
    st.session_state.setdefault(f"{TAB_KEY}_keep_arch", True)
...
def render(ctx: dict | None = None):
    ...
    st.markdown("## 🏡 Real Estate Studio")
    st.caption("Virtual staging (Minimalist/Scandinavian) + lighting fix. Non-blocking job engine.")
    ...
    with c1:
        styles = st.multiselect("Staging style", STYLE_KEYS, key=f"{TAB_KEY}_styles")
        room_type = st.selectbox("Room type", ROOM_TYPES, key=f"{TAB_KEY}_room_type")
        staging_strength = st.selectbox("Staging strength", ["Light", "Medium", "Strong"], key=f"{TAB_KEY}_staging_strength")

    with c2:
        lighting_fix = st.checkbox("Lighting fix (exposure/white balance)", key=f"{TAB_KEY}_lighting_fix")
        declutter = st.checkbox("Declutter / remove mess", key=f"{TAB_KEY}_declutter")
        keep_arch = st.checkbox("Keep architecture (walls/windows/floor)", key=f"{TAB_KEY}_keep_arch")

    with c3:
        st.text_area("Notes (optional)", key=f"{TAB_KEY}_notes", height=110, placeholder="contoh: keep wall color, keep floor material, avoid changing window position")
    ...
    if start_clicked:
        if not uploads:
            st.warning("Upload minimal 1 foto ruangan dulu.")
            st.stop()
        if not styles:
            st.warning("Pilih minimal 1 style staging.")
            st.stop()

        ts = time.strftime("%Y%m%d_%H%M%S")
        job_dir = create_job_dir(ws_root, "real_estate", ts)
        ...
        for idx, f in enumerate(uploads, start=1):
            raw = f.getvalue()
            in_path = inputs_dir / f"room_{idx:02d}_{_slug(getattr(f, 'name', 'upload'))}.png"
        ...
        cfg = {
            ...
            "lighting_fix": bool(lighting_fix),
            "declutter": bool(declutter),
            "keep_arch": bool(keep_arch),
        ...
        worker_py = (Path(__file__).resolve().parents[1] / "tools" / "real_estate_worker.py")
